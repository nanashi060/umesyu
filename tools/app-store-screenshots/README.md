# Umeshu Notes App Store Screenshots Editor

This directory is an isolated copy of the `ParthJadhav/app-store-screenshots` Next.js editor, seeded for Umeshu Notes. It is not part of the Expo app runtime.

From the repository root:

```bash
npm run screenshots:editor:install
npm run screenshots:editor
npm run screenshots:editor:build
```

The canonical editor state is `app-store-screenshots.json`. Uploaded files are stored under `public/screenshots/uploaded/`. Commit both when keeping an edited deck.

The current starter deck references copied ja/en-US iPhone screenshots from `metadata/screenshots/ios`. Review exported editor assets before replacing upload-ready files under `metadata/screenshots`.

Upstream license text is preserved in `LICENSE.upstream`.
